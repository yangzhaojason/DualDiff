...
def run_inference(rank, world_size, model_path, savepath, image_size, overrides):
    ...
    controlnet.use_box_adapter = cfg.use_box_adapter
    ...
    for validation_i in trange(len(idx_per_GPU)):
        ...
        val_input = collate_fn(
            ...
            occ_proj=OccFolderSetWrapper('./occ_proj/occ_bg',mode='image') if cfg.dataset.image_size == [224,400] else OccFolderSetWrapper('./occ_proj/occ_bg_hd',mode='image'),
            cfg=cfg,)
        
        count=0
        # sample_ = nusc.get('sample',val_input['meta_data']['metas'][0].data['token'])
        filenames = nusc[val_input['meta_data']['metas'][0].data['token']]
...
def main():
    ...
    mp.spawn(run_inference, args=(world_size,model_path,savepath,image_size,overrides), nprocs=world_size, join=True)
