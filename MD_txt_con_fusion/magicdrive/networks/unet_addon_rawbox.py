...
from .txt_con_fusion import *
from .box_adapter import *
from ..misc.common import load_module

from collections import OrderedDict
def get_submodel_ckpt(ckpt: OrderedDict, submodel: str):
    res=OrderedDict()
    for i in ckpt.keys():
        if i.startswith(submodel):
            res.update({i[len(submodel)+1:]:ckpt[i]})
    return res
...
class BEVControlNetModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        # combine cam emb into time embedding, create by default, should be deleted if unused
        adm_in_channels = 768+time_embed_dim # cam_emb(768) + t_emb(1280)
        self.adm_proj = torch.nn.Sequential(
            torch.nn.Linear(adm_in_channels, time_embed_dim),
            torch.nn.SiLU(),
            torch.nn.Linear(time_embed_dim, time_embed_dim)
        )
        # text condition fusion, create by default, should be deleted if unused
        self.txt_con_fusion = txt_con_XFormersAttn()
        self.txt_con_fusionp = txt_con_XFormersAttn_plus()
    ...
    def add_cam_states(self, encoder_hidden_states, camera_emb=None):
        ...
        if self.use_aug_text:
            encoder_hidden_states = rearrange(encoder_hidden_states, '(b n) ... -> b n ...',n=N_cam)
        else:
            encoder_hidden_states = repeat(encoder_hidden_states, 'b c ... -> b repeat c ...', repeat=N_cam)
        ...
        # return cam_hidden_states, encoder_hidden_states
    ...
    def from_unet(
        ...
        **kwargs
        ...
        if load_weights_from_unet:
            ...
            else: # init with controlnet_seg
                # in this case the passed in 'seg_init' is actually a seg_controlnet ckpt orderdict
                bev_controlnet.conv_in.load_state_dict(get_submodel_ckpt(seg_init,'conv_in'))
                bev_controlnet.time_proj.load_state_dict(
                    get_submodel_ckpt(seg_init,'time_proj'))
                bev_controlnet.time_embedding.load_state_dict(
                    get_submodel_ckpt(seg_init,'time_embedding')
                )

                if bev_controlnet.class_embedding:
                    bev_controlnet.class_embedding.load_state_dict(
                        get_submodel_ckpt(seg_init,'class_embedding')
                    )

                bev_controlnet.down_blocks.load_state_dict(
                    get_submodel_ckpt(seg_init,'down_blocks'))
                bev_controlnet.mid_block.load_state_dict(
                    get_submodel_ckpt(seg_init,'mid_block'))
    ...
    def add_uncond_to_kwargs(
        ...
        if bboxes_3d_data is None:
            ...
            if isinstance(bboxes_3d_data, list):
                ret["bboxes_3d_data"] = []
                for i in range(len(bboxes_3d_data)):
                    if bboxes_3d_data[i] is None:
                        logging.warn(
                            "Your 'bboxes_3d_data' should not be None. If this warning keeps "
                            "popping, please check your code.")
                        if max_len is not None:
                        ...
                        tmp = dict()  # do not change the original dict
                        for key in ["bboxes", "classes", "masks"]:
                            tmp[key] = torch.cat([
                                torch.zeros_like(bboxes_3d_data[i][key]),
                                bboxes_3d_data[i][key],
                            ])
                            if max_len is not None:
                                token_num = max_len - tmp[key].shape[2]
                                assert token_num >= 0
                                to_pad = torch.zeros_like(tmp[key])
                                to_pad = repeat(
        ...
        if self.uncond_map is None:
            ret['image'] = image  # it actually goes in here which means no change to 'image'
        else:
    ...
    def forward(
        ...
        **kwargs,
        ...
        self.use_aug_text = kwargs['use_aug_text']
        # check channel order
        ...
        if (self.drop_cond_ratio > 0.0 and self.training):
            ...
            uncond_mask = None
        # uncond_mask = None # TODO: how to fix _random_use_uncond_cam
        ...
        for attn_processor in self.attn_processors.values():
            if isinstance(attn_processor, Adapter_XFormersAttnProcessor):
                attn_processor.num_tokens=num_box_tokens
        ...
        bbox_emb = rearrange(
            bbox_emb, 'b n ... -> (b n) ...') if bbox_emb is not None else None
        cls_emb = rearrange(
            cls_emb, 'b n ... -> (b n) ...') if cls_emb is not None else None
        if len(emb) < len(sample):
            emb = repeat(emb, 'b ... -> (b repeat) ...', repeat=N_cam)
        if self.use_cam_in_temb:
            assert False, 'not available now'
            cam_hidden_states = rearrange(
                cam_hidden_states, 'b n ... -> (b n) ...')
            emb = torch.cat([cam_hidden_states, emb], axis=1) # along feature dim
            emb = self.adm_proj(emb)
        else:
            assert self.adm_proj is None
        # controlnet_cond = repeat(
        #     controlnet_cond, 'b ... -> (b repeat) ...', repeat=N_cam)

        # 2. pre-process
        sample = self.conv_in(sample)
        # torch.save(controlnet_cond.detach().cpu(),'before_controlnet_conv_in.pth')
        if not self.use_occ_3d:
            controlnet_cond = self.controlnet_cond_embedding(controlnet_cond)
        else:
            assert self.controlnet_cond_embedding is None
        # torch.save(controlnet_cond.detach().cpu(),'after_controlnet_conv_in.pth')
        assert not (self.use_txt_con_fusion and self.use_txt_con_fusionp) 
        if self.use_txt_con_fusion:
            controlnet_cond = self.txt_con_fusion(
                attn=None,
                hidden_states=controlnet_cond,
                encoder_hidden_states=encoder_hidden_states_with_cam[:,1:]
                )
        else:
            assert self.txt_con_fusion is None
        if self.use_txt_con_fusionp:
            controlnet_cond = self.txt_con_fusionp(
                attn=None,
                hidden_states=controlnet_cond,
                encoder_hidden_states=encoder_hidden_states_with_cam[:,1:]
                )
        else:
            assert self.txt_con_fusionp is None
        # print(sample.min(),sample.max(),controlnet_cond.min(),controlnet_cond.max())
        sample += controlnet_cond

        # if not self.use_cam_in_temb: # TODO
        #     encoder_hidden_states_with_cam = torch.cat([
        #     rearrange(cam_hidden_states.unsqueeze(2), 'b n ... -> (b n) ...'),  # B,N_cam,768 -> B,N_cam,1,768 -> B*N_cam,1,768
        #     encoder_hidden_states_with_cam], dim=1)  # B*N, len + 1, dim
        ...
        for downsample_block in self.down_blocks:
            if (
                ...
                sample, res_samples = downsample_block(
                    ...
                    temb=emb,
                    encoder_hidden_states=torch.cat([_i for _i in [encoder_hidden_states_with_cam, bbox_emb, cls_emb] if _i is not None], dim=1),
        ...
        if not self.use_cam_in_temb:
            encoder_hidden_states_with_cam=torch.cat([
                encoder_hidden_states_with_cam, bbox_emb
                ], dim=1) if bbox_emb is not None else encoder_hidden_states_with_cam
