...
class ContinuousBBoxWithTextEmbedding(nn.Module):
    ...
    def reinitialize(self):
        # re-initialize the following modules if use_map_vec and use_map_vec_40pts
        logging.info("[ContinuousBBoxWithTextEmbedding] Reinitialize 40pts ")
        output_num = 40
        proj_dim = self.bbox_proj.out_features
        self.bbox_proj = nn.Linear(
            self.fourier_embedder.out_dim * output_num, proj_dim)
        self.null_pos_feature = torch.nn.Parameter(
            torch.zeros([self.fourier_embedder.out_dim * output_num]))
    ...
    def forward(self, bboxes: torch.Tensor, classes: torch.LongTensor,
                masks=None, return_cls_emb=False, **kwargs):
        ...
        """
        (B, N) = classes.shape
        bboxes = rearrange(bboxes, 'b n ... -> (b n) ...') # b(batchsize*n_view)*n 8 3
        ...
        if not return_cls_emb:
            return emb
        else:
            cls_emb = rearrange(cls_emb, '(b n) ... -> b n ...', n=N)
            return emb, cls_emb
