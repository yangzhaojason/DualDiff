...
class StableDiffusionBEVControlNetPipeline(StableDiffusionControlNetPipeline):
    ...
    def __call__(
        ...
        if prompt is not None and isinstance(prompt, str):
            ...
            batch_size = len(prompt) if not bev_controlnet_kwargs['use_aug_text'] else len(prompt)//6
        else:
        ...
        if isinstance(image,list):
            ...
            image = self.prepare_image(
                ...
                do_classifier_free_guidance=do_classifier_free_guidance,
                guess_mode=guess_mode,
            )  # (2 * b, c_26, 200, 200) in our case, this function basically only do cat [img]*2 for CFG
        ...
        if do_classifier_free_guidance and not guess_mode:
            # uncond in the front, cond in the tail
            '''skip add_uncond_to_kwargs() with 'image' modification here'''
            # _images = list(torch.chunk(image, 2))
            if isinstance(self.controlnet, MultiControlNetModel):
                kwargs_with_uncond = self.controlnet.nets[0].add_uncond_to_kwargs(
                    camera_param=camera_param,
                    # image=_images[0],  # 0 is for unconditional
                    image=None, # place holder
            ...
            # image = torch.cat(_images)
            del kwargs_with_uncond["image"]
            bev_controlnet_kwargs = move_to(kwargs_with_uncond, self.device)
            bboxes_3d_data = bev_controlnet_kwargs.pop('bboxes_3d_data')
        ###### BEV end ######
