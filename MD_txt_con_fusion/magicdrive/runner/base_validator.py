...
class BaseValidator:
    def __init__(self, cfg, val_dataset, pipe_cls, pipe_param, occ_proj, map_vec) -> None:
        self.cfg = cfg
        ...
        self.occ_proj=occ_proj
        self.map_vec = map_vec
    def validate(
        self,
        ...
        for validation_i in self.cfg.runner.validation_index:
            ...
            val_input = collate_fn(
                ...
                occ_proj=self.occ_proj, map_vec=self.map_vec, cfg=self.cfg
            )
            ...
            for _ in range(self.cfg.runner.validation_times):
                with torch.autocast("cuda"):
                    image: BEVStableDiffusionPipelineOutput = pipeline(
                        ...
                        image=val_input["occ_proj"] if (self.cfg.task_id != '224x400') else val_input["bev_map_with_aux"],
            ...
            # map_img_np = torch.tensor(val_input["bev_map_with_aux"][0])
            # map_img_np = map_img_np.reshape(*map_img_np.shape[:-1],-1,2).permute(3,1,2,0) # 3,224,2400 -> 2,224,1200,3
            # map_img_np = map_img_np.reshape(-1, *map_img_np.shape[2:]).numpy().astype(np.uint8) # 3,2,224,1200 -> 3,448,1200
            # # map_img_np = map_img_np.reshape(map_img_np.shape[0],map_img_np.shape[1]*2,map_img_np.shape[2]//2) # 3,224,2400 -> 3,448,1200
            # map_img_np = to_pil_image(map_img_np)
            # gen_wb_list.append(map_img_np)
            image_logs.append(
                {
