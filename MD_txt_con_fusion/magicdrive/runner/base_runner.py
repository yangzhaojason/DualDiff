...
class BaseRunner:
    def __init__(self, cfg, accelerator, train_set, val_set) -> None:
        ...
        for i in range(num_branch):
            ...
            self.occ_proj.append(_occ_proj)
            self.map_vec.append(_map_vec)
        self.occ_proj = self.occ_proj[0] if not cfg.use_dual_controlnet else self.occ_proj
        self.map_vec = self.map_vec[0] if not cfg.use_dual_controlnet else self.map_vec
        ...
        self.validator = BaseValidator(
            ...
            occ_proj=self.occ_proj,
            map_vec =self.map_vec,
        )
    ...
    def _set_xformer_state(self):
        ...
        if self.cfg.runner.enable_xformers_memory_efficient_attention:
            if is_xformers_available():
                ...
                # self.controlnet_refer.enable_xformers_memory_efficient_attention()
            else:
                raise ValueError(
    ...
    def _validation(self, step):
        if isinstance(self.controlnet, list):
            ...
            for c_net in self.controlnet:
                controlnet.append(self.accelerator.unwrap_model(c_net))
        else:
            controlnet = self.accelerator.unwrap_model(self.controlnet)
